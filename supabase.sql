...
-- Phase 2: monthly divisional leaderboards read straight from
-- handicap_scores, no new table needed there. The daily top-3 auto-post
-- (fired from a handicap sync) needs a per-day lock so it only posts once
-- regardless of how many friends sync that day.
create table if not exists public.daily_leaderboard_posts (
